...
# # # Set to 1 to use narrow precision compression (7-bit exponent, bias 116, range 1024 to 10^-35)
# # # Set to 0 to use standard precision compression (8-bit exponent, bias 127)
...
# #                     exp_f = 0;
# #                     mant_trunc = 0;
# #                 }} else if (biased_f >= (1 << NUM_EXP_BITS)) {{
# #                     exp_f = (1U << NUM_EXP_BITS) - 1;  // Max exponent
# #                     mant_trunc = 0;
# #                 }} else {{
...
# #                             mant_trunc = 0; exp_f++;
# #                             if (exp_f >= (1U << NUM_EXP_BITS)) {{ exp_f = (1U << NUM_EXP_BITS) - 1; mant_trunc = 0; }}
# #                         }}
# #                     }}
# #                 }}
# #             }}
...
# #             unsigned int max_exp_f = (1U << NUM_EXP_BITS) - 1;
...
# #                     // Clamp to valid double exponent range to prevent overflow
# #                     if (exp_d > 1022) exp_d = 1022;
# #                 }}
# #             }}
...
#         // Combine both components into a single bit stream
#         // Real in lower T bits, imaginary in upper T bits
#         unsigned long long combined = (packed_vals[1] << T) | packed_vals[0];

#         // Write combined bit stream to shared memory
#         for (size_t i = 0; i < num_bytes; i++) {{
#             s_out[threadIdx.x * num_bytes + i] = (combined >> (8 * i)) & 0xFF;
#         }}
#     }}
#     __syncthreads();
...
#     if (idx < N) {{
#         // Read combined bit stream
#         unsigned long long combined = 0;
#         size_t base_s_idx = threadIdx.x * num_bytes;
#         for (size_t i = 0; i < num_bytes; i++) {{
#             combined |= (unsigned long long)s_inp[base_s_idx + i] << (8 * i);
#         }}

#         // Extract real and imaginary components from combined bit stream
#         unsigned long long real_bits = combined & ((1ULL << T) - 1);
#         unsigned long long imag_bits = (combined >> T) & ((1ULL << T) - 1);
